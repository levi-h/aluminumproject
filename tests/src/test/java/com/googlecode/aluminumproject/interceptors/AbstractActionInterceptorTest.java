...
public class AbstractActionInterceptorTest {
	...
	public void actionPhasesFromConstructorShouldBeIntercepted() {
		Set<ActionPhase> phases = new DefaultActionInterceptor(ActionPhase.CREATION, ActionPhase.EXECUTION).getPhases();
		assert phases != null;
		assert phases.size() == 2;
		assert phases.contains(ActionPhase.CREATION);
